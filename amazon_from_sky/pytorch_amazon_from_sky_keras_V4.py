...
#%%
def image_generator(base_folder, csv_file, folder_name, batch_size=64):

    labels_df = pd.read_csv(base_folder+csv_file)
    image_list = [f for f in glob.glob(base_path+folder_name+'/*.jpg')]
...
#%%
for i in range(4):
    X, y = next(image_generator(base_path, 'train_v2.csv', 'train-jpg', batch_size=16))
